...
\abstract{This vignette presents the gmm4 package, which is an attempt
...
properties of $g_i(\theta$). The GMM model class will only include the
definition of $g_i(\theta)$ and its assumed statistical properties,
which is basically represented by its variance.
...
\item[a)] ``iid'': Here we assume no autocorrelation and homoscedastic
  ...
  and $\hat{\epsilon}_i=Y_i-X_i'\hat{\theta}(W)$.
...
\item The functional or formula case: If we cannot represent the model
  ...
  worth making the distinction because we can easily show that the
  centered optimal weighting matrix in that case does not depend on
  the coefficient $\theta$. Efficient GMM can therefore be estimated
  in one step.
...
course, it makes no sense when $g_i(\theta)$ is a general function.

The main difference is the slots that define $g_i(\theta)$. For
``linearGmm'' class, the slots ``modelF'' and ``instF'' are
model.frame's that define the regression model and the
instruments. For ``nonlinearGmm'', we have the following slots:
...
function D() can be used to obtain analytical derivatives. The class
``formulaGmm'' is similar to the ``nonlinearGMM'' class with the
exception that the slots ``fRHS'' and ``fLHS'' are lists of
...
indicates if is it an MDE model. Finally, the ``functionGmm'' class
...
errors. We could create an object of class ``linarGmm'' as follows:
...
constructor is the function ``gmmModel''. The above
...
``vcovOptions''. For example, if we want an HAC matrix, several options
such as the kernel and bandwidth can be modified. By default, the HAC
is computed using the Quadratic Spectral kernel the optimal bandwidth
of \cite{andrews91}. To modify the options, we proceed this way:
...
\begin{itemize}
\item \textit{residuals}: Only for linearGMM and nonlinearGMM, it
  returns $\epsilon(\theta)$:
...
\item \textit{Dresiduals}: Only for linearGMM and nonlinearGMM, it
  returns the $n\times k$ matrix $d\epsilon(\theta)/d\theta$:
...
\item \textit{model.matrix}: For linearGMM and nonlinearGmm only. For
  both classes, it ca be used to get the matrix of instruments:
...
\item \textit{modelResponse}: For linear model only, it returns the
  vector of response. It is not defined for nonlinearGMM classes
...
\item \textit{as}: linearGmm can be converted into a nonlinearGmm or
  functionGmm. The former is userful when we impose nonlinear
...
\item \textit{momentVcov}: It computes $\hat{V}$ using the
...
\item \textit{momentStrength}: For linearGmm only (for now), it
...
\item \textit{update}: This method is used to modify existing
  objects. For now, only the covariance structure can be modified. We
  could, for example, change the covariance structure of mod1 from
  ``iid'' to ``MSD'':
...
We can create objects of class ``rlinearGmm'', ``rnonlinearGMM'' or
``rfunctionGMM'' using the method \textit{restModel} and print the
restrictions using the \textit{printRestrict} method.
...
For nonlinearGmm, only character vectors or lists of formulas are
...
class object plus a few additional slots. We can therefore use the
\textit{as} method directly to convert a restricted model to its
unrestricted counterpart. We can therefore compute the residuals from
...
and \textit{momentVcov}. The methods that will produce different
...
estimation more computationally efficient and more numerically stable,
...
q$ matrix $Z$. The ``gmmWeights'' class store only what is needed. It
can be created by the \textit{evalWeights} method. It is a method for
the union class ``gmmModels'', which includes all restricted
models.The method has three arguments, the ``gmmModels'', the vector
of coefficients, and the type of weights. The third argument can be a
matrix, if we want to provide our own fixed one, the character
"ident", to create an identity matrix or, which is the default, the
character "optimal". In the latter case, the efficient weighting
matrix is computed based on the characteristics of the ``gmmModels''
...
The \textit{show} method for the ``gmmWeights'' object prints the
...
``gmmModels'' including ``functionGmm'' classes, with vcov="MDS", the
...
object. Finally, the "[" method can be used to create another
  ``gmmWeights'' object with a subset of the moment conditions. Only
...
\textit{evalObjective} method. In this case, the weights is not
...
We now have all we need to estimate our models. The main method to
estimate a model for a given $W$ is \textit{solveGmm}. The available
signatures are:
...
The last three are for systems of equations that we will cover
later. The first is for ``nonlinearGmm'' and ``functionGmm'', and the
second for ``linearGmm''. The methods require a gmmWeights object as
second argument. For ``nonlinearGmm'', ``functionGmm'' and
``formulaGmm'' classes, there are two more optional arguments. The
first is ``theta0'', which is the starting value to pass to the
minimization algorithm. If not provided, the one stored in the GMM
model object is used. The second is ``algo'' which specifies which
algorithm to use to minimize the objective function. By default,
\textit{optim} is used. The only other choice for now is
\textit{nlminb}. The default method for \textit{optim} is ``BFGS'',
but all arguments of the algorithm can be modified by specifying them
directly in the call of \textit{solveGmm}.

The method simply minimizes $\bar{g}(\theta)'W\bar{g}(\theta)$ for a
given $W$. For ``linearGmm'' classes, the analytical solution is
used. It is therefore the prefered class to use when it is
possible. For all other classes, the solution is obtained by the
selected algorithm. For ``nonlinearGmm'' and ``formulaGmm'', the
...
\subsection{GMM Estimation: the \textit{modelFit} method} \label{sec:gmmmodels-gmmfit}
...
is done. The \textit{modelFit} method is the main method to estimate
models. The only requirement is to first create a
``gmmModels''. Before going into all the details, the most important
arguments to set is the object, which is a ``gmmModels'' class, and a
type of GMM. The different types are: (1) ``twostep'' for two-step
GMM, which is the default, (2) ``iter'' for iterative GMM, (3) ``cue''
for continuously updated GMM , or (4) ``onestep'' for the one-step GMM.
...
applies to all ``gmmModels''. If $q=k$, the model is
just-identified. In that case, in theory, the choice of $W$ has no
effect on the solution. Therefore, by default, \textit{modelFit} will
...
conditions complicates the problem. The \textit{modelFit} method
...
``nonlinearGmm'' model, the matrices $W_1$ and $W_2$ are proportional
...
two-stage least squares solution. In fact, \textit{modelFit} calls the
method \textit{tsls} in that case. We will look at the method below.

The \textit{modelFit} method returns the S4 class object
``gmmfit''. The object contains the vector of coefficient estimates,
the ``gmmWeights'' used to obtain it, the model object and other
...
\textit{modelFit} returns the one-step GMM. Alternatively, we can
provide \textit{modelFit} with a fixed weighting matrix. It could be a
matrix or a ``gmmWeights'' object. When the weighting matrix is
...
before and pass it to the \textit{modelFit} method. The value of the
``efficientGmm'' slot will be used by the \textit{vcov} method to
determine whether it should return the robust (or sandwich) covariance
matrix.

There is a specific \textit{modelFit} method for ``formulaGmm''
classes. It behaves differently only if ``weights'' is set to
``optimal'' and the slot ``isMDE'' of the object is TRUE. The
\textit{gmmModel} constructor detects if the right-hand-side or the
...
on the coefficients, and call the general \textit{modelFit} method
...
\item \textit{vcov}: It returns the covariance matrix of the
  ...
  \begin{itemize}
    ...
    \end{itemize}
  The method is flexible enough that you may hand up with a non-valid
  ...
  not estimated by efficient GMM. It is important to understand that
  we assume here that the specifications of the model are valid. If
  you set ``vcov'' to iid and that the errors are heteroscedastic, GMM
  is no longer efficient and a sandwich matrix is required. But event
  if you set ``sandwich'' to TRUE, it will not solve the problem
  because the meat will be computed assuming the errors are iid. You
  can, however, set the argument ``modelVcov'' to ``MDS'' which will
  set ``sandwich'' to TRUE and compute the meat properly.
...
matrix is based on the specification of the ``gmmModels''. For
...
If the model has iid error, \textit{modelFit} and \textit{tsls} are
numerically identical. In fact, the function is called by
\textit{modelFit} in that case. The main reason for using it is if we
...
If you still think that the \textit{modelFit} method is not simple
...
\textit{gmmModel} constructor and \textit{modelFit} method. Here are a
few examples.
...
The two classes are ``slinearGmm'' and ``snonlinearGmm'' and the union
class is ``sgmmModels''. For most, the slots are the same with the
...
``slinearGmm'', the equations and instruments are defined in the slots
``modelT'' and ``instT'', the latter being also the format for
``snonlinearGmm'' classes. They are lists of terms for each
formula. There are two extra slots in system classes, ``eqnNames'',
which labels each equation, and ``SUR'', which is TRUE if the SUR
assumption is satisfied.The constructor is \textit{sysGmmModel} and
works as the \textit{gmmModel} constructor. A \textit{show} method
...
conditions variance. ``snonlinearGmm'' are constructed the same way
...
create a ``linearGmm'' class using a matrix as the left hand side of
...
``gmmModels'' classes. Here, we briefly describe the difference.
...
\item \textit{momentVcov}: It returns the $Q\times Q$ matrix
...
class is ``rslinearGmm'' and it contains its unrestricted
...
applied to ``rslinearGmm'' objects. When a vector of coefficient is
...
\textit{evalMoment} and \textit{momentVcov}. All derivatives methods,
...
of \textit{residuals} and \textit{momentVcov}. For example, the
residuals:
...
The \textit{"["} method works the same way. We can therefore get the
  first equation as a ``rlinearGmm'' object as follows:
...
of equations is ``sysGmmWeights''.  The simplest weighting matrix is
...
\subsection{The \textit{modelFit} method for system of equations}\label{sec:sgmmmodels-gmmfit}
...
matrix or a ``sysGmmWeights'' object, the method will return a one
...
instead of \textit{modelFit} is that the latter is an efficient GMM,
...
have covered 2SLS cases in Section \ref{sec:gmmmodels-app}. First we
