...
\Address{
  ...
  E-mail: \email{pchausse@uwaterloo.ca}
}
\begin{document}
...
We want to estimate a vector of parameters $\theta_0 \in \Real^p$ from a model based on the following $q\times 1$ vector of unconditional moment conditions:
...
\left\{
\begin{array}{lcr}
\exp{(-\gamma^\alpha|\tau_l|^\alpha[1-i\beta(\tan{\frac{\pi\alpha}{2}})(\mathrm{sign}(\tau_l))]
  + i\delta \tau_l)} &\mbox{for}&\alpha\neq
1\\ \exp{(-\gamma|\tau_l|[1+i\beta\frac{2}{\pi}(\mathrm{sign}(\tau_l))\log{|\tau_l|}]
  + i\delta \tau_l)} &\mbox{for}&\alpha= 1\\
\end{array}
\right. ,
\]
